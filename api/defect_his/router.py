...
def list_(
    ...
    defect_code_mst_id: int | None = None,
):
    q = db.query(DefectHisModel)
    if prod_his_id is not None:
        q = q.filter(DefectHisModel.prod_his_id == prod_his_id)
    if defect_code_mst_id is not None:
        q = q.filter(DefectHisModel.defect_code_mst_id == defect_code_mst_id)
...
def create(p: DefectHisCreate, db: Session = Depends(get_db)):
    row = DefectHisModel(
        ...
        defect_code_mst_id=p.defect_code_mst_id,
