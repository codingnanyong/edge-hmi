...
def list_(
    ...
    q = db.query(LineMstModel)
    if process_type is not None:
        q = q.filter(LineMstModel.process_type == process_type)
    return q.order_by(LineMstModel.id).offset(skip).limit(limit).all()
...
def update(id: int, p: LineMstUpdate, db: Session = Depends(get_db)):
    ...
    if not row:
        raise HTTPException(404, "line_mst not found")
    if p.process_type is not None:
        row.process_type = p.process_type
